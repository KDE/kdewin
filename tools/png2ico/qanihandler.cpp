...
/*! \class QtAniHandler
    ...
    \brief The QtAniHandler class provides support for the ICO image format.
...
typedef struct _IconSubchunk
{
    ...
    quint32 Size;         /* Length of subchunk minus 8 */
    uchar  Data[];       /* Icon, cursor, or raw bitmap data */ 
} ICONSUBCHUNK;
...
typedef struct _AniHeaderSubchunk
{
    ...
    quint32 Size;         /* Length of subchunk minus 8 */
    quint32 HeaderSize;   /* Size of the subchunk data in bytes */
    quint32 NumFrames;    /* Number of icon or cursor frames */
    quint32 NumSteps;     /* Number of steps in the animation */
    quint32 Width;        /* Width of frame in pixels */
    quint32 Height;       /* Height of frame in pixels */
    quint32 BitCount;     /* Number of bits in the frame pixels */
    quint32 NumPlanes;    /* Number of color planes in the frame data */
    quint32 DisplayRate;  /* Default frame display rate */
    quint32 Flags;        /* File attributes flags */
} ANIHEADERSUBCHUNK;
#define ANIHEADERSUBCHUNK_SIZE 44

typedef struct _ListFrameChunk
{
    char        ListId[4];     /* "LIST" */
    quint32        ListSize;   /* Length of LIST chunk minus 8 */
    char        ListType[4];   /* "fram" */
//     ICONSUBCHUNK Frames[];   /* Array of cursor, icon, or bitmap data */
} LISTFRAMECHUNK;
#define LISTFRAMECHUNK_SIZE 12


typedef struct _RiffAniFile
{
    char              FileId[4];     /* File ID (always "RIFF") */
    quint32              Size;       /* Length of file minus 8 in bytes */
...
public:
    ...
    static bool write(QIODevice * device, const QList<QImage> & images, int hotspotx, int hotspoty, int framerate);
...
static bool writeIconDir(QIODevice *iodev, const ICONDIR &iconDir)
{
    if (iodev) {
        ...
        qToLittleEndian(1, &tmp[4]);
        int ret = iodev->write((char*)tmp,  6);
        return (ret == 6) ? true : false;
    }
    return false;
}

static bool writeIconDirEntry(QIODevice *iodev, const ICONDIRENTRY &iconEntry, int hotspotx, int hotspoty)
{
    if (iodev) {
        uchar tmp[ICONDIRENTRY_SIZE];
        ...
        qToLittleEndian<quint16>(hotspotx, &tmp[4]); //hotspot x
        qToLittleEndian<quint16>(hotspoty, &tmp[6]); //hotspot y
...
ANIReader::ANIReader(QIODevice * iodevice)
: iod(iodevice)
, startpos(0)
, headerRead(false)
{
...
bool ANIReader::canRead(QIODevice *iodev)
{
...
bool ANIReader::readIconEntry(int index, ICONDIRENTRY * iconEntry)
{
    if (iod) {
...
bool ANIReader::readBMPHeader(ICONDIRENTRY & iconEntry, BMP_INFOHDR * header)
{
...
void ANIReader::findColorInfo(QImage & image)
{
...
void ANIReader::readColorTable(QImage & image)
{
    if (iod) {
...
void ANIReader::readBMP(QImage & image)
{
 ...
 *
 */
void ANIReader::read1BitBMP(QImage & image)
{
    if (iod) {
...
void ANIReader::read4BitBMP(QImage & image)
{
    if (iod) {
...
void ANIReader::read8BitBMP(QImage & image)
{
    if (iod) {
...
void ANIReader::read16_24_32BMP(QImage & image)
{
    if (iod) {
...
QImage ANIReader::iconAt(int index)
{
    QImage img;
...
/*!
    ...
    \sa read()
*/
bool ANIReader::write(QIODevice * device, const QList<QImage> & images, int hotspotx, int hotspoty, int framerate)
{
    bool retValue = false;
    ...
    if (images.count()) {
        ...
        for (int i=0; i<id.idCount; i++) {
            ...
            entries[i].wPlanes = 1;
            entries[i].dwImageOffset = origOffset + ICONDIR_SIZE + (1 * ICONDIRENTRY_SIZE);
        ...
        af.AniHeader.NumSteps = id.idCount;
        af.AniHeader.Width = 0;
        af.AniHeader.Height = 0;
        af.AniHeader.BitCount = 0;
        af.AniHeader.NumPlanes = 0;
        ...
        for (int i = 0; i < id.idCount; i++) {
            uchar tmp[8];
            memcpy(&tmp, "icon", 4);
            qToLittleEndian<quint32>(imageData[i].size()+BMP_INFOHDR_SIZE+ICONDIRENTRY_SIZE+6, &tmp[4]);
            bOk &= (device->write((char*)&tmp,  sizeof(tmp)) == sizeof(tmp)) ? true : false;
            bOk &= writeIconDir(device, id);
            bOk &= writeIconDirEntry(device, entries[i], hotspotx, hotspoty);
...
/*!
    Constructs an instance of QtAniHandler initialized to use \a device.
*/
QtAniHandler::QtAniHandler(QIODevice *device)
{
    m_currentIconIndex = 0;
    setDevice(device);
    m_pANIReader = new ANIReader(device);
}

/*!
    Destructor for QtAniHandler.
*/
QtAniHandler::~QtAniHandler()
{
    delete m_pANIReader;
}
 ...
 * If the magic bytes were found, it is assumed that the QtAniHandler can read the file.
 *
 */
bool QtAniHandler::canRead() const
{
    bool bCanRead = false;
    QIODevice *device = QImageIOHandler::device();
    if (device) {
        bCanRead = ANIReader::canRead(device);
        if (bCanRead)
            setFormat("ico");
    } else {
        qWarning("QtAniHandler::canRead() called with no device");
    }
    return bCanRead;
}
...
*/
bool QtAniHandler::canRead(QIODevice *device)
{
    Q_ASSERT(device);
    return ANIReader::canRead(device);
}
...
{
    bool bSuccess = false;
    QImage img = m_pANIReader->iconAt(m_currentIconIndex);
...
*/
bool QtAniHandler::write(const QImage &image, int hotspotx, int hotspoty, int framerate)
{
    QIODevice *device = QImageIOHandler::device();
    QList<QImage> imgs;
    imgs.append(image);
    return ANIReader::write(device, imgs, hotspotx, hotspoty, framerate);
}

bool QtAniHandler::write(const QList<QImage> &images, int hotspotx, int hotspoty, int framerate)
{
    QIODevice *device = QImageIOHandler::device();
    return ANIReader::write(device, images, hotspotx, hotspoty, framerate);
}
 ...
 */
QByteArray QtAniHandler::name() const
{
    return "ico";
}
...
*/
int QtAniHandler::imageCount() const
{
    return m_pANIReader->count();
}

/*! \reimp

*/
bool QtAniHandler::jumpToImage(int imageNumber)
{
...
*/
bool QtAniHandler::jumpToNextImage()
{
