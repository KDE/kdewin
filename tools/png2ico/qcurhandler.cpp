...
/*! \class QtCurHandler
    ...
    \brief The QtCurHandler class provides support for the ICO image format.
...
public:
    ...
    static bool write(QIODevice * device, const QList<QImage> & images, int hotspotx, int hotspoty);
...
bool CURReader::readBMPHeader(ICONDIRENTRY & iconEntry, BMP_INFOHDR * header)
{
...
/*!
    ...
    \sa read()
*/
bool CURReader::write(QIODevice * device, const QList<QImage> & images, int hotspotx, int hotspoty)
{
    bool retValue = false;
...
/*!
    Constructs an instance of QtCurHandler initialized to use \a device.
*/
 ...
 * If the magic bytes were found, it is assumed that the QtCurHandler can read the file.
 *
 */
...
*/
bool QtCurHandler::write(const QImage &image, int hotspotx, int hotspoty)
{
    QIODevice *device = QImageIOHandler::device();
    QList<QImage> imgs;
    imgs.append(image);
    return CURReader::write(device, imgs, hotspotx, hotspoty);
}

bool QtCurHandler::write(const QList<QImage> &images, int hotspotx, int hotspoty)
{
    QIODevice *device = QImageIOHandler::device();
    return CURReader::write(device, images, hotspotx, hotspoty);
}
